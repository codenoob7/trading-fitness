...
if __name__ == "__main__":
    # Quick test with a simple declining NAV
    print("=== Bear ITH Algorithm Test ===")
    print()

    # Test case: pure decline (should produce bear epochs)
    nav_decline = np.array(
        [100.0, 98.5, 97.0, 99.0, 96.5, 94.0, 95.5, 92.0, 90.0, 88.0, 85.0, 83.0, 80.0],
        dtype=np.float64,
    )
    hurdle = 0.05

    result = bear_excess_gain_excess_loss(nav_decline, hurdle)
    print(f"NAV: {nav_decline}")
    print(f"Max runup: {max_runup(nav_decline):.4f}")
    print(f"Bear epochs: {result.num_of_bear_epochs}")
    print(f"Epoch indices: {list(np.where(result.bear_epochs)[0])}")
    print(f"Bear intervals CV: {result.bear_intervals_cv:.4f}")
    print()

    # Test case: pure rally (should produce NO bear epochs)
    nav_rally = np.array(
        [
            100.0,
            102.0,
    ...
    result_rally = bear_excess_gain_excess_loss(nav_rally, hurdle)
    print(f"NAV (rally): {nav_rally}")
    print(f"Max runup: {max_runup(nav_rally):.4f}")
    print(f"Bear epochs: {result_rally.num_of_bear_epochs}")
    print(f"Epoch indices: {list(np.where(result_rally.bear_epochs)[0])}")
